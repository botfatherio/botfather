...
class AdbDeviceInfo {
   public:
    AdbDeviceInfo();
    AdbDeviceInfo(const QString &serial_number, const QString &state,
   ...
   private:
    // A string created by adb to uniquely identify the device by its port
    ...
    // - device: The device is now connected to the adb server. Note that this
    // state does not imply that the Android system is fully
    //   booted and operational because the device connects to adb while the
    //   system is still booting. However, after boot-up, this is the normal
