...
#include "../browser/browser_util.hpp"
#include "../engine.hpp"
...
QScriptValue BrowserPrototype::constructor(QScriptContext *context,
                                           QScriptEngine *engine) {
    // Each bot/engine instance has its own browsers. When creating a browser we
    // must tell the browser host which group/bot it belongs in. The group name
    // used here is the bots ID.
    ...
    if (context->argumentCount() >= 1 && context->argument(0).isString()) {
        ...
        if (id.isEmpty()) {
            return context->throwError(QScriptContext::Error::TypeError,
                                       "The browsers id must not be empty");
        }
    }
    ...
    if (context->argumentCount() == 1 && context->argument(0).isString()) {
        ...
        Browser *browser = BrowserManager::instance()->createBrowser(
            vm->id(), id, default_size);
        return engine->newQObject(browser, QScriptEngine::QtOwnership);
    }

    // new Browser(id, size);
    if (context->argumentCount() == 2 && context->argument(0).isString() &&
        isQSize(context->argument(1))) {
        QSize size = qscriptvalue_cast<QSize>(context->argument(1));
        if (size.width() < Browser::MIN_WIDTH ||
            size.height() < Browser::MIN_HEIGHT) {
            return context->throwError(
                QScriptContext::Error::RangeError,
                "The browsers size must be at least 400x400");
        }
        Browser *browser =
            BrowserManager::instance()->createBrowser(vm->id(), id, size);
        return engine->newQObject(browser, QScriptEngine::QtOwnership);
    }
...
QRect BrowserPrototype::getRect() const { return THIS_BROWSER_P()->rect(); }

QSize BrowserPrototype::getSize() const { return THIS_BROWSER_P()->size(); }

void BrowserPrototype::resize(const QSize &size) {
    if (size.width() < Browser::MIN_WIDTH ||
        size.height() < Browser::MIN_HEIGHT) {
        context()->throwError(QScriptContext::Error::RangeError,
                              "The browsers size must be at least 400x400");
        return;
    }
...
void BrowserPrototype::replaceResource(const QString &resource_pattern,
...
void BrowserPrototype::resetUserAgent() { THIS_BROWSER_P()->resetUserAgent(); }
...
QUrl BrowserPrototype::getUrl() const { return THIS_BROWSER_P()->url(); }

void BrowserPrototype::reload() { THIS_BROWSER_P()->reload(); }
...
void BrowserPrototype::stopLoading() { THIS_BROWSER_P()->stopLoading(); }
...
void BrowserPrototype::goForward() { THIS_BROWSER_P()->goForward(); }

void BrowserPrototype::goBack() { THIS_BROWSER_P()->goBack(); }
...
void BrowserPrototype::scrollWheel(const QPoint &position,
                                   const QPoint &delta) {
...
void BrowserPrototype::pressKey(const QString &keycode) {
    if (!THIS_BROWSER_P()->canPressKey(keycode)) {
        QString message = QString(
            "Unsupported keycode. Check the keymap in the Browser "
            "documentation");
...
void BrowserPrototype::enterText(const QString &text) {
    if (!THIS_BROWSER_P()->canEnterText(text)) {
        QString message = QString(
            "text contains unsupported characters. Check the keymap in the "
...
QScriptValue BrowserPrototype::evaluateJavascript(
    ...
    if (THIS_BROWSER_P()->evaluateJavascript(script_url, javascript_code,
                                             timeout_in_ms, result, exception,
                                             timed_out)) {
        ...
        QString msg = QString("[%1] Line %2, %3")
                          .arg(script_url)
                          .arg(line_number)
                          .arg(message);
        ...
        QString msg(
            QString("Evaluation timed out, more that %0 milliseconds passed.")
                .arg(timeout_in_ms));
        return context()->throwError(QScriptContext::UnknownError, msg);
    } else {
...
QString BrowserPrototype::toString() const {
    return QString("Browser(id: %0, size: %1x%2)")
        .arg(THIS_BROWSER_P()->name())
        .arg(THIS_BROWSER_P()->size().width(),
             THIS_BROWSER_P()->size().height());
}
