...
class ImagePrototype : public AbstractPrototype {
   ...
   public:
    ...
    // Returns true if the image has a format that respects the alpha channel,
    ...
    // Load image data from a file (relative paths are considered relative to
    // the script) https://doc.qt.io/qt-5/qimage.html#load
    ...
    // Turns all pixels black which are not in range of min and max. Pixels in
    // range keep their color or will be turned white depending on keep_color.
    Q_INVOKABLE QImage isolateColorRange(const QColor &min, const QColor &max,
                                         bool keep_color = false);

    // Returns the number of pixels different on two images of the same size.
    // Compares two same sized images and returns the number pixels they don't
    // have in common.
    Q_INVOKABLE int countDifferentPixels(const QImage &other_image) const;

    // Calculates the percentage of pixels being the same color and on the same
    // position on both images. Both images must have the same size.
