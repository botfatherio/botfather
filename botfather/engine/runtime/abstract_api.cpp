...
AbstractAPI::AbstractAPI(Engine *bot, QObject *parent)
    : QObject(parent), bot_p(bot) {}

Engine *AbstractAPI::bot() const { return bot_p; }
...
void AbstractAPI::enable(QScriptEngine *engine, QString api_name) {
    // Note: The engine pointer argument is required because engine() returns a
    // nullptr if the Qt function was not invoked from script code.
    QScriptValue api_js_obj(
