...
BlobTpl::BlobTpl()
{
	blob_params.filterByColor = true; // Always filter by color. Color means black and white...	
	...
	// 0 <= minArea <= maxArea
	blob_params.filterByArea = true; // Always filter by area. Not doing so makes no sense.
...
void BlobTpl::setMinCircularity(double new_min_circularity)
{
	...
	blob_params.filterByCircularity = blob_params.minCircularity >= 0 && blob_params.maxCircularity >= 0;
}
...
void BlobTpl::setMinConvexity(double new_min_convexity)
{
	...
	blob_params.filterByConvexity = blob_params.minConvexity >= 0 && blob_params.maxConvexity >= 0;
}
...
void BlobTpl::setMinInertia(double new_min_inertia_ratio)
{
	...
	blob_params.filterByInertia = blob_params.minInertiaRatio >= 0 && blob_params.maxInertiaRatio >= 0;
}
...
{
	blob_params.minRepeatability = static_cast<unsigned long>(new_min_repeatability);
}
