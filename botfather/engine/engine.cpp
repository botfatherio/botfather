...
#include "runtime/algorithm_api.hpp"
#include "runtime/android_api.hpp"
#include "runtime/blob_tpl_prototype.hpp"
#include "runtime/browser_prototype.hpp"
#include "runtime/color_prototype.hpp"
#include "runtime/config_api.hpp"
#include "runtime/desktop_api.hpp"
#include "runtime/helper_api.hpp"
#include "runtime/image_prototype.hpp"
#include "runtime/margins_prototype.hpp"
#include "runtime/match_prototype.hpp"
#include "runtime/point_prototype.hpp"
#include "runtime/rect_prototype.hpp"
#include "runtime/size_prototype.hpp"
#include "runtime/stats_api.hpp"
#include "runtime/timer_prototype.hpp"
#include "runtime/url_prototype.hpp"
#include "runtime/vision_api.hpp"

Engine::Engine(const QString &id, const QString &script_path,
               const QString &config_ini_path)
    : m_id(id), script_path(script_path), m_config_ini_path(config_ini_path) {
    // Otherwise Bot::LogSource can't be used with slots
    qRegisterMetaType<Engine::LogSource>("Engine::LogSource");

    // The script engines parent MUST BE the bot instance. This way we can
    // obtain a pointer to the bot instance in static functions (like
    // constructors) by casting the engines parent to a Bot*.
    script_engine = new QScriptEngine(this);
}

Engine::~Engine() {
    // The QJSEngine (not QScriptEngine) docs state that the garbage is not
    // collected when the engine gets destroyed. The advise to call
    // collectGarbage before deleting it.
    script_engine->collectGarbage();
    // delete script_engine; // has bot as parent.
}

QString Engine::id() const { return m_id; }
...
QString Engine::configIniPath() const { return m_config_ini_path; }
...
void Engine::runScript() {
    ...
    if (!script_file.open(QIODevice::ReadOnly)) {
        QString debug_msg("Can't read the contents from " + script_path +
                          ". Please check the files permissions.");
    ...
    // Check whether the script ended due to errors. If so print them to the
    // users log.
    if (result.isError()) {
        QString error_msg = replaceQtWithEngineTypeNames(result.toString());
        QString debug_msg = QString("Line %1, %2")
...
void Engine::stop() { script_engine->abortEvaluation(); }

QString Engine::replaceQtWithEngineTypeNames(QString text) {
    // Ignore reference errors: "ReferenceError: Can't find variable:
    // QSomethingThatDoesNotExist"
    ...
    // Note: Replace pointer types before non pointer types. Otherwise the "*"
    // will not be replaced.

    // Detect "QTypeName" and "QTypeName*" and make "TypeName" be in the first
    // capture group. The first capture group can be refered to with "\\1".
