...
void Desktop::warpCursor(const QPoint &position) {
    ...
    // The following trick doesn't work: We can't move the mouse relatively -9999 -9999
    // to get the mouse to the top left corner of the screen.
    // But moving it then by 100 100 doesn't result in the mouse
    // being at 100 100 unfortunately. For some reason it works when the mouse
    // has been placed at 0 0 before manually.
    // Fortunately, the Xlib allows us to warp the pointer/cursor to a absolute position.
    // But mouseinput generated by Xlib is flagged as fake input and thus ignored by some
