...
bool UrlPrototype::isEmpty() const
{
	return THIS_URL().isEmpty();
}

bool UrlPrototype::isValid() const
{
	return THIS_URL().isValid();
}

QString UrlPrototype::getScheme() const
{
	return THIS_URL().scheme();
}
...
void UrlPrototype::setHost(const QString &host)
{
	THIS_URL_P()->setHost(host);
}

int UrlPrototype::getPort() const
{
	return THIS_URL().port(-1);
}

void UrlPrototype::setPort(int port)
{
	THIS_URL_P()->setPort(port);
}
...
void UrlPrototype::setPath(const QString &path)
{
	THIS_URL_P()->setPath(path);
}

bool UrlPrototype::hasQuery() const
{
	return THIS_URL().hasQuery();
}
...
bool UrlPrototype::hasFragment() const
{
	return THIS_URL().hasFragment();
}
...
{
	// Usually toString returns formated information about the object like this: Point(x: 100, y: 100)
	// An url on the other hand has a logical string representation, thus we return it unformated.
