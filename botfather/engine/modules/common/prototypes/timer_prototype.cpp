...
QScriptValue TimerPrototype::constructor(QScriptContext *context,
...
bool TimerPrototype::isRunning() const { return THIS_TIMER().isValid(); }

void TimerPrototype::start() { THIS_TIMER_P()->start(); }
...
void TimerPrototype::stop() { THIS_TIMER_P()->invalidate(); }
...
qint64 TimerPrototype::msecsTo(const QElapsedTimer &other) const {
    // impl => The return value is undefined if this object or other were
    // invalidated.
    QElapsedTimer timer(THIS_TIMER());
...
QString TimerPrototype::toString() const {
    return QString("Timer(running: %1, elapsed: %2)")
