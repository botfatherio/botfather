...
class TimerPrototype : public AbstractPrototype {
   ...
   public:
    ...
    // The methods of http://doc.qt.io/qt-5/qelapsedtimer.html have been
    // modified. Methods which require the QElapsedTimer to be valid return a
    // default value if the timer isn't valid. And both invalidate() and
    // isValid() have been renamed, as their names might confuse newcomers.

    // Returns false if the timer has never been started or stopped by a call to
    // stop(). http://doc.qt.io/qt-5/qelapsedtimer.html#isValid
    ...
    // The return value is 0 if this object is invalidated, the timer though
    // will be started anyway. http://doc.qt.io/qt-5/qelapsedtimer.html#restart
    ...
    // started. The return value is 0 if this object is invalidated.
    ...
    // value of timeout can be -1 to indicate that this timer does not expire,
