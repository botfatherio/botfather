...
QScriptValue SizePrototype::constructor(QScriptContext *context,
...
bool SizePrototype::isEmpty() const { return THIS_SIZE().isEmpty(); }

int SizePrototype::getWidth() const { return THIS_SIZE().width(); }

void SizePrototype::setWidth(int width) { THIS_SIZE_P()->setWidth(width); }

int SizePrototype::getHeight() const { return THIS_SIZE().height(); }

void SizePrototype::setHeight(int height) { THIS_SIZE_P()->setHeight(height); }
...
void SizePrototype::transpose() { THIS_SIZE_P()->transpose(); }

QSize SizePrototype::transposed() const { return THIS_SIZE().transposed(); }

QString SizePrototype::toString() const {
    QSize size = THIS_SIZE();
    return QString("Size(width: %1, height: %2)")
        .arg(size.width())
        .arg(size.height());
}
