...
QScriptValue MarginsPrototype::constructor(QScriptContext *context,
...
bool MarginsPrototype::isNull() const { return THIS_MARGINS().isNull(); }

int MarginsPrototype::getTop() const { return THIS_MARGINS().top(); }

void MarginsPrototype::setTop(int top) { THIS_MARGINS_P()->setTop(top); }

int MarginsPrototype::getLeft() const { return THIS_MARGINS().left(); }

void MarginsPrototype::setLeft(int left) { THIS_MARGINS_P()->setLeft(left); }

int MarginsPrototype::getRight() const { return THIS_MARGINS().right(); }

void MarginsPrototype::setRight(int right) {
    THIS_MARGINS_P()->setRight(right);
}

int MarginsPrototype::getBottom() const { return THIS_MARGINS().bottom(); }
...
QMargins MarginsPrototype::marginsSubtracted(
    const QMargins &other_margins) const {
    return THIS_MARGINS() - other_margins;
}

QString MarginsPrototype::toString() const {
    QMargins margins = THIS_MARGINS();
    return QString("Margins(left: %1, right: %2, top: %3, bottom: %4)")
        .arg(margins.left())
        .arg(margins.right())
        .arg(margins.top())
        .arg(margins.bottom());
