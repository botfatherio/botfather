...
QScriptValue BlobTplPrototype::constructor(QScriptContext *context,
    ...
    // BlobTpl(min_area, max_area, min_circularity, max_circularity,
    // min_convexity, max_convexity, min_inertia, max_inertia, min_threshold,
    ...
    if (context->argumentCount() >= 2) {
        ...
        if (min_area > max_area) {
            return RANGE_ERROR(context,
                               "min_area must be smaller or equal to max_area");
        }
    ...
    if (context->argumentCount() >= 4) {
        ...
        if (min_circularity > max_circularity) {
            return RANGE_ERROR(
                context,
                "min_circularity must be smaller or equal to max_circularity");
        }
        if (max_circularity > 1.0) {
            return RANGE_ERROR(
                context, "max_circularity must be smaller or equal to 1.0");
        }
    ...
    if (context->argumentCount() >= 6) {
        ...
        if (min_convexity > max_convexity) {
            return RANGE_ERROR(
                context,
                "min_convexity must be smaller or equal to max_convexity");
        }
        if (max_convexity > 1.0) {
            return RANGE_ERROR(context,
                               "max_convexity must be smaller or equal to 1.0");
        }
    ...
    if (context->argumentCount() >= 8) {
        ...
        if (min_inertia > max_inertia) {
            return RANGE_ERROR(
                context, "min_inertia must be smaller or equal to max_inertia");
        }
        if (max_inertia > 1.0) {
            return RANGE_ERROR(context,
                               "max_inertia must be smaller or equal to 1.0");
        }
    ...
    if (context->argumentCount() >= 10) {
        ...
        if (min_threshold > max_threshold) {
            return RANGE_ERROR(
                context,
                "min_threshold must be smaller or equal to max_threshold");
        }
        if (max_threshold > 255) {
            return RANGE_ERROR(context,
                               "max_threshold must be smaller or equal to 255");
        }
    ...
    if (context->argumentCount() == 11) {
        ...
        if (threshold_step > max_threshold - min_threshold) {
            return RANGE_ERROR(
                context,
                "threshold_step must be smaller or equal to max_circularity");
        }
...
QString BlobTplPrototype::toString() const {
    ...
    return QString(
               "BlobTpl(area: %1-%2, circ: %3-%4, conv: %5-%6, iner: %7-%8, "
               "thre: %9-%10, step: %11)")
        .arg(blob_tpl.getMinArea())
        .arg(blob_tpl.getMaxArea())
        .arg(blob_tpl.getMinCircularity())
        .arg(blob_tpl.getMaxCircularity())
        .arg(blob_tpl.getMinConvexity())
        .arg(blob_tpl.getMaxConvexity())
        .arg(blob_tpl.getMinInertia())
        .arg(blob_tpl.getMaxInertia())
        .arg(blob_tpl.getMinThreshold())
        .arg(blob_tpl.getMaxThreshold())
