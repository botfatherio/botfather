...
QScriptValue ImagePrototype::constructor(QScriptContext *context,
                                         QScriptEngine *engine) {
    // Instead of casting the engines parent to a Bot* we could expose required
    // bot functionality via the HelperAPI to engine and call it from here.
    ...
    if (context->argumentCount() == 1 && context->argument(0).isString()) {
        ...
        if (!image.load(absolute_path)) {
            QString message =
                QString("can't load image from path %1").arg(absolute_path);
    ...
    if (context->argumentCount() == 2 && isQSize(context->argument(0)) &&
        isQColor(context->argument(1))) {
...
bool ImagePrototype::isNull() const { return THIS_IMAGE().isNull(); }

bool ImagePrototype::hasAlphaChannel() const {
    return THIS_IMAGE().hasAlphaChannel();
}

int ImagePrototype::getWidth() const { return THIS_IMAGE().width(); }

int ImagePrototype::getHeight() const { return THIS_IMAGE().height(); }

QSize ImagePrototype::getSize() const { return THIS_IMAGE().size(); }
...
void ImagePrototype::fill(const QColor &color) { THIS_IMAGE_P()->fill(color); }

QColor ImagePrototype::getPixelColor(const QPoint &position) const {
    return THIS_IMAGE().pixelColor(position);
}

void ImagePrototype::setPixelColor(const QPoint &position,
                                   const QColor &color) {
...
QImage ImagePrototype::createMaskFromColor(const QColor &color) {
    QImage mask =
        THIS_IMAGE().createMaskFromColor(color.rgb(), Qt::MaskOutColor);
...
QImage ImagePrototype::isolateColorRange(const QColor &min, const QColor &max,
                                         bool keep_color) {
    ...
    cv::Mat result_mat =
        Vision::isolateColor(image_mat, min_hsv, max_hsv, keep_color);
...
double ImagePrototype::pixelEquality(const QImage &other_image) const {
    ...
    int equal_pixels =
        total_pixels - Vision::countDifferentPixels(image_1, image_2);
...
QString ImagePrototype::toString() const {
    ...
    return QString("Image(width: %1, height: %2, bytes: %3)")
        .arg(image.width())
        .arg(image.height())
        .arg(ImageSizeInBytes(image));
}
