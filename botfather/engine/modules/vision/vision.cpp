...
cv::Mat Vision::isolateColor(const cv::Mat &image, const cv::Scalar &min_hsv,
                             const cv::Scalar &max_hsv, bool keep_color) {
    ...
    cv::cvtColor(image, hsv_umat, cv::COLOR_BGR2HSV);
    ...
    cv::cvtColor(hsv_umat, bgr_result_umat, cv::COLOR_HSV2BGR);
...
int Vision::countDifferentPixels(const cv::Mat &image_1,
...
double Vision::histogramSimilarity(const cv::Mat &image_1,
                                   const cv::Mat &image_2) {
    cv::Mat hsv_1, hsv_2;
    cv::cvtColor(image_1, hsv_1, cv::COLOR_BGR2HSV);
    cv::cvtColor(image_2, hsv_2, cv::COLOR_BGR2HSV);
    ...
    cv::calcHist(&hsv_1, 1, channels, cv::Mat(), hsv_1_hist, dims, hist_size,
                 ranges, true, false);
    cv::normalize(hsv_1_hist, hsv_1_hist, 0, 1, cv::NORM_MINMAX, -1, cv::Mat());

    cv::calcHist(&hsv_2, 1, channels, cv::Mat(), hsv_2_hist, dims, hist_size,
                 ranges, true, false);
    ...
    return cv::compareHist(hsv_1_hist, hsv_2_hist, cv::HISTCMP_CORREL);
}

QVector<Match> Vision::findMaskedMatches(const cv::Mat &image,
                                         const cv::Mat &tpl,
                                         const cv::Mat &mask, double threshold,
                                         int max_matches) {
    // Note: Only CV_TM_SQDIFF and CV_TM_CCORR_NORMED accept maskes.
    static const int match_method = cv::TM_CCORR_NORMED;
    bool match_method_accepts_mask =
        (cv::TM_SQDIFF == match_method || match_method == cv::TM_CCORR_NORMED);
    ...
    // Create a result matrix of the images size. NOTE: Saving the result image
    // doesn't work for some reason. But We tested saving it using:
    // cv::imwrite("result.png", result.getMat(cv::ACCESS_READ).clone()); But
    // result.png is always black, no matter where in this method we tried to
    // save it.
    cv::UMat uresult(uimage.rows - utpl.rows + 1, uimage.cols - utpl.cols + 1,
                     CV_32FC1);

    // cv::matchTemplate requires the image to have a depth of 0 or 5, and both
    // the image and the template to gave the same types. The images dims must
    // be equal or less 2.
    Q_ASSERT(uimage.depth() == 0 || uimage.depth() == 5);
    Q_ASSERT(uimage.type() == utpl.type());
    Q_ASSERT(uimage.dims <= 2);

    // Marks spots on the result mat than lighten than the template matches on
    // the image. When a mask is used ignore those regions which are black on
    // the mask.
    ...
    // Makes not so good results better which results in "better" matches in
    // some cases but in wrong matches if there is actually nothing to match.
    ...
    // thresholding.
    cv::threshold(uresult, uresult, threshold, 1.0, cv::THRESH_TOZERO);
    ...
    while (matches.size() < max_matches || max_matches == -1) {
        ...
        cv::minMaxLoc(uresult, &min_val, &max_val, &min_loc, &max_loc,
                      cv::Mat());
        ...
        // Stop looking for more matching when the matches threshold becomes to
        // low.
        if (match_val < threshold) {
            break;
        }

        // Make the currently lightest (best matching) location black, so it
        // won't be found again while looking for the next best match. One can
        // use floodfill to do so, which should work as expected, especially
        // after thresholding. BUT we had some issues doing so! (Matches where
        // found twice or more. With a different configuration the whole result
        // image turned black after the first match). Drawing a circle where the
        // lightes match is turned out to be more reliable. NOTE: Whatever
        // method is used, one has to respect the user match method as different
        // methods mark the best matching location either black OR white.
        ...
        // Format the match and push it to the other matches. x and y of
        // match_loc are the left and top coordinates of the match. Note: in our
        // Match class x and y mark the center of the match.
        matches.push_back(Match(
            QRect(match_loc.x, match_loc.y, utpl.cols, utpl.rows), match_val));
    }
...
Match Vision::findMaskedMatch(const cv::Mat &image, const cv::Mat &tpl,
                              const cv::Mat &mask, double threshold) {
    QVector<Match> matches =
        Vision::findMaskedMatches(image, tpl, mask, threshold, 1);
    if (!matches.isEmpty()) {
        return matches[0];
    }
    return Match();
...
QVector<Match> Vision::findMatches(const cv::Mat &image, const cv::Mat &tpl,
                                   double threshold, int max_matches) {
    return Vision::findMaskedMatches(image, tpl, cv::Mat(), threshold,
                                     max_matches);
}

Match Vision::findMatch(const cv::Mat &image, const cv::Mat &tpl,
                        double threshold) {
    return Vision::findMaskedMatch(image, tpl, cv::Mat(), threshold);
}

QVector<Match> Vision::findBlobs(
    const cv::Mat &image, const cv::SimpleBlobDetector::Params &blob_params) {
    cv::UMat image_umat;
...
QImage Vision::markMatches(const QImage &image, const QVector<Match> &matches,
...
QImage Vision::cvMatToQImage(const cv::Mat &mat) {
    switch (mat.type()) {
        ...
        default:
            qWarning() << "cv::Mat image type not handled in switch:"
                       << mat.type();
            break;
    }
