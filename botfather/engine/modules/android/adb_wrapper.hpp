...
class AdbWrapper : public QObject {
    Q_OBJECT
   public:
    explicit AdbWrapper(QObject *parent = nullptr,
                        const QString &adb_binary = QString());
   ...
   public:
    ...
    bool sendSwipe(QString serial_number, int x1, int y1, int x2, int y2,
                   int duration_in_ms);
    bool sendKeyEvent(
        QString serial_number,
        QString
            key_event_code);  // https://developer.android.com/reference/android/view/KeyEvent.html
    bool sendEvent(QString serial_number, QString device, int type, int code,
                   int value);
   ...
   private:
    ...
    bool evaluateAdbCommand(QStringList arguments, QString &standard_output,
                            int timeout_in_ms = 30000);
    QProcess::ProcessError executeAdbCommand(QStringList arguments,
