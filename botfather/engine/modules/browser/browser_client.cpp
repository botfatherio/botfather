...
bool BrowserClient::OnProcessMessageReceived(
    ...
    if (message->GetName() == "eval_javascript_result") {
        QString result_id = QString::fromStdString(
            message->GetArgumentList()->GetString(0).ToString());
        bool success = message->GetArgumentList()->GetBool(1);

        // Not initialising these caused segmentation faults in slots receiving
        // the evalJavascriptResultReady signal. When they copied the
        // uninitialised values they caused the application to crash from time
        // to time.
        ...
        if (success) {
            CefRefPtr<CefBinaryValue> binary_result =
                message->GetArgumentList()->GetBinary(2);
            QCborStreamReader cbor_stream_reader(
                BrowserUtil::convertCefBinaryValueToQByteArray(binary_result));
            ...
            CefRefPtr<CefBinaryValue> binary_exception =
                message->GetArgumentList()->GetBinary(3);
            QDataStream data_stream(
                BrowserUtil::convertCefBinaryValueToQByteArray(
                    binary_exception));
