...
class BrowserRequestHandler : public CefRequestHandler {
   public:
    ...
    // Makes the browser replace any resource matching the 'resource_pattern'
    // with 'replacement_url'.
   ...
   private:
    ...
    // Contains all redirected urls. The first item of a pair is a regex pattern
    // matching the original url. The second item is either empty, meaning the
    // url should not be loaded (is blocked), or there is a second url provided,
    // meaning the second url should be loaded instead.
