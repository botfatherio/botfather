...
class BrowserRenderHandler : public CefRenderHandler {
   public:
    ...
    explicit BrowserRenderHandler(BrowserClient *parent,
    ...
    // moved or becomes focused. (Multiple BrowserWindows can display the same
    // Browser).
    ...
    // Called to allow the client to fill in the CefScreenInfo object with
    // appropriate values.
    bool GetScreenPoint(CefRefPtr<CefBrowser> browser, int view_x, int view_y,
