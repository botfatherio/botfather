...
class BrowserManager {
   public:
    ...
    // Spawns required subprocesses. This must be called right after starting
    // the application. Returns true when the initialisation succeeded otherwise
    // false.
    ...
    Browser *createBrowser(const QString &group_name, const QString &browser_id,
                           const QSize &size);

    // Deletes a Browser created using the .createBrowser() method after
    // removing it from the BrowserListModel and correctly freeing the
    // underlying CefBrowser pointer.
