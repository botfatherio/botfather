...
BrowserRenderHandler::BrowserRenderHandler(BrowserClient *parent,
    ...
    : m_parent(parent), m_size(browser_size) {}
...
bool BrowserRenderHandler::GetRootScreenRect(CefRefPtr<CefBrowser> browser,
                                             CefRect &rect) {
    // If this method returns false the rectangle from GetViewRect will be used.
    return false;
}

bool BrowserRenderHandler::GetScreenInfo(CefRefPtr<CefBrowser> browser,
...
bool BrowserRenderHandler::GetScreenPoint(CefRefPtr<CefBrowser> browser,
                                          int view_x, int view_y, int &screen_x,
                                          int &screen_y) {
...
void BrowserRenderHandler::GetViewRect(CefRefPtr<CefBrowser>, CefRect &rect) {
    m_size_lock.lockForRead();
    rect.Set(m_screen_point_offset.x(), m_screen_point_offset.y(),
...
void BrowserRenderHandler::OnPaint(CefRefPtr<CefBrowser> browser,
    ...
    // QPixmap is stored on the XServer when using X11 backend, whereas QImage
    // is just an "array in memory" of the client program. So drawing QPixmaps
    // on XWindow systems should be much faster than drawing QImages, as the
    // data is already on the server, processed and ready to use.

    m_screenshot_lock.lockForWrite();
    m_screenshot = QImage(static_cast<const unsigned char *>(buffer), width,
