...
void BrowserUtil::runAfterQtEventLoopStarted(std::function<void()> callback) {
    // This singleShot timers start will be scheduled in the apps Qt message
    // loop. Thus, as soon as the message loop starts the callback will be
    // called.
    QTimer::singleShot(0, callback);
}

QByteArray BrowserUtil::convertCefBinaryValueToQByteArray(
...
QScriptValue BrowserUtil::convertToQScriptValue(QScriptEngine *engine,
    ...
    if (cbor_value.isArray()) {
        ...
        QScriptValue array =
            engine->newArray(static_cast<uint>(cbor_array.size()));

        for (quint32 i = 0; i < cbor_array.size(); ++i) {
            array.setProperty(
                i, BrowserUtil::convertToQScriptValue(engine, cbor_array[i]));
        }
        return array;
    }
    ...
    if (cbor_value.isMap()) {
        ...
        for (QCborValue key : cbor_map.keys()) {
            ...
            object.setProperty(
                key.toString(),
                BrowserUtil::convertToQScriptValue(engine, cbor_map[key]));
        }
        return object;
    }
