...
BrowserListModel *BrowserManager::model() const { return m_model; }
...
bool BrowserManager::init(int argc, char **argv) {
    ...
    return CefInitialize(main_args, cefSettings(), browser_app.get(),
                         sandbox_info);
}
...
void BrowserManager::bind(QCoreApplication *app) {
    BrowserUtil::runAfterQtEventLoopStarted([app, this]() {
        ...
        QObject::connect(m_cef_message_loop_timer, &QTimer::timeout,
                         CefDoMessageLoopWork);
        QObject::connect(app, &QCoreApplication::aboutToQuit,
                         [this]() { this->quit(); });
        QObject::connect(app, &QCoreApplication::aboutToQuit,
                         m_cef_message_loop_timer, &QTimer::stop);
        QObject::connect(app, &QCoreApplication::aboutToQuit,
                         m_cef_message_loop_timer, &QTimer::deleteLater);
...
CefSettings BrowserManager::cefSettings() const {
    ...
    QString subprocess_path = QStandardPaths::findExecutable(
        "webhelper", {QApplication::applicationDirPath()});
    CefString(&cef_settings.browser_subprocess_path)
        .FromASCII(subprocess_path.toUtf8());

    CefString(&cef_settings.resources_dir_path) =
        CefString();  // cef.pak devtools_resources.pak
    CefString(&cef_settings.locales_dir_path) = CefString();

    // Define a cache dir WHICH IS ABSOLUTELY REQUIRED because some site might
    // not work otherwise. IMPORTANT: Requires an absolute path otherwise the
    // program will crash.
    QString absolute_cache_path =
        QStandardPaths::writableLocation(QStandardPaths::CacheLocation);
    CefString(&cef_settings.cache_path) =
        CefString(absolute_cache_path.toStdString());

    // This option is recommended for use in combination with the
    // DoMessageLoopWork() function in cases where the CEF message loop must be
    // integrated into an existing application message loop
    // cef_settings.external_message_pump = true;
    // While this option is the recommended way in our case, it cause the
    // problems described in the following issue:
    // https://bitbucket.org/chromiumembedded/cef/issues/2340/x11-3239-paste-from-system-clipboard-does
    // However, not setting this option to true, did not cause any problems so
    // far.
...
Browser *BrowserManager::createBrowser(const QString &group_name,
                                       const QString &browser_id,
                                       const QSize &size) {
    QModelIndexList matches = m_model->match(
        m_model->index(0, 0), m_model->BROWSER_UID_ROLE,
        m_model->createUID(group_name, browser_id), 1, Qt::MatchExactly);

    if (!matches.isEmpty()) {
        QVariant variant =
            m_model->data(matches.first(), m_model->BROWSER_PTR_ROLE);
        Browser *browser = qvariant_cast<Browser *>(variant);
        Q_ASSERT(browser);

        qDebug() << "Returning existing browser" << browser->group()
                 << browser->name();
        return browser;
    }
...
void BrowserManager::closeCefBrowser(CefRefPtr<CefBrowser> browser) {
    ...
    if (!CefCurrentlyOn(TID_UI)) {
        BrowserUtil::runInMainThread([browser]() { closeCefBrowser(browser); });
        return;
    }
    ...
    BrowserClient *browser_client =
        static_cast<BrowserClient *>(cef_client.get());
