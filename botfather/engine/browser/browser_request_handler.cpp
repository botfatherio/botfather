...
BrowserRequestHandler::BrowserRequestHandler(BrowserClient *parent)
    : m_parent(parent) {}
...
void BrowserRequestHandler::replaceResource(const QString &resource_pattern,
...
void BrowserRequestHandler::resetUserAgent() { m_user_agent_override.clear(); }

CefRequestHandler::ReturnValue BrowserRequestHandler::OnBeforeResourceLoad(
    ...
    if (!m_user_agent_override.isEmpty()) {
        ...
        std::multimap<CefString, CefString>::iterator user_agent_match =
            header_map.find("User-Agent");
...
void BrowserRequestHandler::OnPluginCrashed(CefRefPtr<CefBrowser> browser,
                                            const CefString &plugin_path) {
    qDebug() << "Plugin Crashed:"
             << QString::fromStdString(std::string(plugin_path));
    emit m_parent->pluginCrashedSignal();
}

void BrowserRequestHandler::OnRenderProcessTerminated(
