...
class Engine : public QObject {
   ...
   public:
    // Giving Bot a parent results in: QObject::moveToThread: Cannot move
    // objects with a parent
    Engine(const QString &id, const QString &script_path,
           const QString &config_ini_path);
    ~Engine();

    enum LogSource { System, Error, Script, Debug };

    QString id() const;

    // Considers relative paths relative to the scripts dir and makes them
    // absolute.
