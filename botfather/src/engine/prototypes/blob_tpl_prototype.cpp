...
/*
void BlobTplPrototype::setArea(int new_area)
{
	THIS_BLOB_TPL_P()->setArea(new_area);
}

int BlobTplPrototype::getArea() const
{
	return THIS_BLOB_TPL().getArea();
}

void BlobTplPrototype::setCircularity(float new_circularity)
{
	THIS_BLOB_TPL_P()->setCircularity(new_circularity);
}

float BlobTplPrototype::getCircularity() const
{
	return THIS_BLOB_TPL().getCircularity();
}

void BlobTplPrototype::setConvexity(float new_convexity)
{
	THIS_BLOB_TPL_P()->setConvexity(new_convexity);
}

float BlobTplPrototype::getConvexity() const
{
	return THIS_BLOB_TPL().getConvexity();
}

void BlobTplPrototype::setInertiaRatio(float new_inertia_ratio)
{
	THIS_BLOB_TPL_P()->setInertia(new_inertia_ratio);
}

float BlobTplPrototype::getInertiaRatio() const
{
	return THIS_BLOB_TPL().getInertia();
}

void BlobTplPrototype::setThreshold(int new_threshold)
{
	THIS_BLOB_TPL_P()->setThreshold(new_threshold);
}

int BlobTplPrototype::getThreshold() const
{
	return THIS_BLOB_TPL().getThreshold();
}

QString BlobTplPrototype::toString() const
{
	BlobTpl blob_tpl = THIS_BLOB_TPL();
	return QString("BlobTpl(area: %1, circularity: %2, convexity: %3, inertia: %4, threshold: %5)")
		.arg(blob_tpl.getArea())
		.arg(static_cast<double>(blob_tpl.getCircularity()))
		.arg(static_cast<double>(blob_tpl.getConvexity()))
		.arg(static_cast<double>(blob_tpl.getInertia()))
		.arg(blob_tpl.getThreshold());
}
*/