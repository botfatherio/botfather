...
bool AdbWrapper::queryForDevices(QList<AdbDeviceInfo> &devices)
{
	...
	QString standard_output; // "List of devices attached\n31464723a1f4d623\tdevice\n\n"
	bool result = evaluateAdbCommand(args, standard_output);

	QRegularExpression regex("^(?<serial_number>\\S*)(\\s*)(?<state>no device|device|offline)(.*)product:(?<product>\\S*)(.*)model:(?<model>\\S*)(.*)$");
...
{
	QStringList args;
	args << "-s" << serial_number << "shell" << "monkey" << "-p" << package << "-c" << "android.intent.category.LAUNCHER" << "1";
	return evaluateAdbCommand(args);
}
...
bool AdbWrapper::sendTap(QString serial_number, int x, int y)
{
	...
	QStringList args;
	args << "-s" << serial_number << "shell" << "input" << "tap" << QString::number(x) << QString::number(y);
	return evaluateAdbCommand(args);
}

bool AdbWrapper::sendSwipe(QString serial_number, int x1, int y1, int x2, int y2, int duration_in_ms)
{
	QStringList args;
	args << "-s" << serial_number << "shell" << "input" << "touchscreen" << "swipe" << QString::number(x1) << QString::number(y1)
		 << QString::number(x2) << QString::number(y2) << QString::number(duration_in_ms);
	return evaluateAdbCommand(args);
}
...
{
	QStringList args;
	args << "-s" << serial_number << "shell" << "input" << "text" << text;
	return evaluateAdbCommand(args);
}
...
bool AdbWrapper::evaluateAdbCommand(QStringList arguments, QString &standard_output, int timeout_in_ms)
{
	QByteArray std_out;
	QByteArray std_err;
	QProcess::ProcessError result = executeAdbCommand(arguments, std_out, std_err, timeout_in_ms);
	standard_output = std_out;
	return std_err.isEmpty() && result == QProcess::ProcessError::UnknownError;
}

QProcess::ProcessError AdbWrapper::executeAdbCommand(QStringList arguments, QString &standard_output, QString &standard_error, int timeout_in_ms){
	QByteArray std_out;
	QByteArray std_err;
	QProcess::ProcessError result = executeAdbCommand(arguments, std_out, std_err, timeout_in_ms);
...
QProcess::ProcessError AdbWrapper::executeAdbCommand(QStringList arguments, QByteArray &standard_output, QByteArray &standard_error, int timeout_in_ms)
{
	...
	// QProcess::ProcessError::UnknownError is the default return value of QProcess::error()
	// even if there was no error and shall thus not been threaded as an real error.
