...
int handleXError(Display *display, XErrorEvent *event) {
    ...
    XGetErrorText(display, event->error_code, error_code_text_buffer,
                  sizeof(error_code_text_buffer));

    // TODO: Test whether using XGetErrorText with request codes is appropriate
    char request_code_text[1024];
    XGetErrorText(display, event->error_code, request_code_text,
                  sizeof(request_code_text));

    // TODO: Test whether using XGetErrorText with minor codes is appropriate
    char minor_code_text[1024];
    XGetErrorText(display, event->minor_code, minor_code_text,
                  sizeof(request_code_text));

    qDebug() << "X error received. type: " << event->type
             << ", serial: " << event->serial
...
int main(int argc, char *argv[]) {
    ...
    // QSettings from all over the app. Change this to match the bots
    // credentials.
    ...
    QSettingsJsonFormat::setFormat(
        QSettings::registerFormat("json", &QSettingsJsonFormat::readFunc,
                                  &QSettingsJsonFormat::writeFunc));
    ...
    // Init libgit2 once and never shut it down. Shutting libgit2 down caused
    // crashes related to multithreading even when compiling libgit2 to be
    // thread safe. The libgit2 docs state some interesting things:
    /* 1. (Thats a CMAKE flag ->) THREADSAFE. Selects whether libgit2 **tries**
     * to be threadsafe
     * - https://libgit2.org/docs/guides/build-and-link/
     * 2. Usually you don’t need to call the shutdown function as the operating
    ...
    // Runs the QApplication event loop blocking. When the event loop stops the
    // timer powering the CEF event loop will stop aswell. After that the CEF
    // can be shut down calling Browser::quit(). Subsequently both cef and qt
    // event and message loops stopped, the applications exit code can be
    // returned.
    return a.exec();
}
