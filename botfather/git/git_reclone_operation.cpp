#include "git_reclone_operation.hpp"
...
static void moveDir(const QString &source_path, const QString &dest_path) {
    ...
    foreach (QFileInfo info,
             source_dir.entryInfoList(QDir::AllEntries | QDir::NoDotAndDotDot |
                                      QDir::Hidden)) {
...
void GitRecloneOperation::replaceRepo() {
    ...
    files_to_preserve << ".settings.ini"
                      << ".config.json";

    for (const QString &file_name : files_to_preserve) {
        qDebug() << "Trying to preserve" << file_name;
        QFile::copy(repo_dir.filePath(file_name),
                    m_tmp_clone_dir.filePath(file_name));
    }

    // Replace the bot folders contents with the updated contents including the
    // preserved files
