...
void GitCloneOperation::process() {
    ...
    clone_opts.checkout_opts.checkout_strategy =
        GIT_CHECKOUT_FORCE;  // git checkout --force; all modifications are
                             // overwritten, and all missing files are created.
    clone_opts.checkout_opts.progress_cb =
        &AbstractGitOperation::checkoutProgressCallback;
    clone_opts.checkout_opts.progress_payload = this;

    int return_code =
        git_clone(&repo, m_repo_url.toUtf8(), m_dir_path.toUtf8(), &clone_opts);
