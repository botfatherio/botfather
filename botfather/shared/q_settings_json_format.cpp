...
QSettings::Format QSettingsJsonFormat::format() { return m_format; }

void QSettingsJsonFormat::setFormat(const QSettings::Format &format) {
    m_format = format;
}

bool QSettingsJsonFormat::readFunc(QIODevice &device,
    ...
    QJsonDocument json_document =
        QJsonDocument::fromJson(json_text.toUtf8(), &parse_error);
...
bool QSettingsJsonFormat::writeFunc(QIODevice &device,
    ...
    // Don't close the device, causes crash.
    return device.write(json_text) !=
           -1;  // return code is -1 if an error occurred
}
