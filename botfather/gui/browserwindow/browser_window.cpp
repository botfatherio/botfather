...
BrowserWindow::BrowserWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::BrowserWindow)
{
	...
	/*
	connect(ui->actionReload, &QAction::triggered, OldBrowser::reload);
	connect(ui->actionStop, &QAction::triggered, OldBrowser::stopLoad);
	connect(ui->actionBack, &QAction::triggered, OldBrowser::goBack);
	connect(ui->actionForward, &QAction::triggered, OldBrowser::goForward);
	
	// Update our navigation buttons every time the browser clients loading state changes.
	connect(OldBrowserClient::instance(), SIGNAL(loadingStateChangedSignal(bool)), this, SLOT(updateNavigationButtons(bool)));
	*/
}
...
void BrowserWindow::applyResizing()
{
	...
	OldBrowser::resize(QSize(width, height));
	...
	// being async.
	connect(OldBrowserClient::instance(), SIGNAL(paintSignal(QImage)), this, SLOT(filterOldSize(QImage)));
	*/
}
...
void BrowserWindow::filterOldSize(QImage browser_image)
{
	...
	if (browser_image.size() == this->pixmap_placeholder->size()) {
		...
		disconnect(OldBrowserClient::instance(), SIGNAL(paintSignal(QImage)), this, SLOT(filterOldSize(QImage)));
		connect(OldBrowserClient::instance(), SIGNAL(paintSignal(QImage)), this, SLOT(paintSlot(QImage)));
	}
	*/
}
...
{
	//OldBrowser::loadUrl("about:version");
}

void BrowserWindow::updateNavigationButtons(bool browser_loading_state)
{
	/*
	this->ui->actionBack->setEnabled(OldBrowser::canGoBack());
	this->ui->actionForward->setEnabled(OldBrowser::canGoForward());
...
void BrowserWindow::showEvent(QShowEvent *event)
{
	...
	//QObject::connect(OldBrowserClient::instance(), SIGNAL(paintSignal(QImage)), this, SLOT(paintSlot(QImage)));
}
...
void BrowserWindow::hideEvent(QHideEvent *event)
{
	...
	//QObject::disconnect(OldBrowserClient::instance(), SIGNAL(paintSignal(QImage)), this, SLOT(paintSlot(QImage)));
}
...
void BrowserWindow::resizeEvent(QResizeEvent *event)
{
	...
	/*
	if (disconnect(OldBrowserClient::instance(), SIGNAL(paintSignal(QImage)), this, SLOT(paintSlot(QImage)))){
