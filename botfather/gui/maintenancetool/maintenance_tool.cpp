#include "maintenance_tool.hpp"

#include <QDebug>
#include <QStandardPaths>

MaintenanceTool::MaintenanceTool(QObject *parent) : QObject(parent) {}

QString MaintenanceTool::filePath() {
    return QStandardPaths::findExecutable("maintenancetool", {"./"});
}

bool MaintenanceTool::exists() { return !filePath().isEmpty(); }

void MaintenanceTool::checkForUpdates() {
    QProcess *process = getPreparedProcess();
    connect(
        process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
        [this, process](int, QProcess::ExitStatus exit_status) {
