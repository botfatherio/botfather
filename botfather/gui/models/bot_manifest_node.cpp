...
BotManifestNode::BotManifestNode(const QJsonObject &json_object,
                                 const QString &config_path)
    : m_json_object(json_object), m_config_path(config_path) {}

bool BotManifestNode::isNull() const { return m_json_object.isEmpty(); }

QVariant BotManifestNode::getVariantByKey(const QString &property_key) const {
    if (!jsonObject().contains(property_key) ||
        jsonObject()[property_key].isUndefined()) {
        return QVariant();
    }
...
QString BotManifestNode::getStringBykey(const QString &property_key) const {
    if (!jsonObject().contains(property_key) ||
        !jsonObject()[property_key].isString()) {
        return QString();
    }
...
int BotManifestNode::getIntegerByKey(const QString &property_key) const {
    if (!jsonObject().contains(property_key) ||
        !jsonObject()[property_key].isDouble()) {
        return 0;
    }
...
QJsonObject BotManifestNode::jsonObject() const { return m_json_object; }

void BotManifestNode::setJsonObject(const QJsonObject &json_object) {
    m_json_object = json_object;
}

QString BotManifestNode::configPath() const { return m_config_path; }
