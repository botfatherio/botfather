...
bool BotConfigOption::isValid() const { return !id().isEmpty(); }

bool BotConfigOption::isStored() const {
    QSettings config(configPath(), QSettingsJsonFormat::format());

    // Don't retrive the value using BotConfigSetting::value as it provides a
    // fallback
...
QString BotConfigOption::id() const { return getStringBykey("id"); }
...
QString BotConfigOption::label() const { return getStringBykey("label"); }
...
QMap<QString, QString> BotConfigOption::choices() const {
    ...
    if (!jsonObject().contains("choices") ||
        !jsonObject()["choices"].isObject() ||
        jsonObject()["choices"].isArray()) {
...
bool BotConfigOption::hasRange() const {
    return jsonObject().contains("min") && jsonObject().contains("max") &&
           jsonObject()["min"].isDouble() && jsonObject()["max"].isDouble();
}

int BotConfigOption::min() const { return getIntegerByKey("min"); }

int BotConfigOption::max() const { return getIntegerByKey("max"); }
