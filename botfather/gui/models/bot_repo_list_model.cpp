...
BotRepoListModel::BotRepoListModel(QObject *parent)
...
QVariant BotRepoListModel::data(const QModelIndex &index, int role) const {
    ...
    if (role == KeywordsRole) {
        ...
        keyword_parts << bot_repo.name() << bot_repo.developer()
                      << bot_repo.description();
    ...
    if (role == Qt::DisplayRole) {
        switch (index.column()) {
            case 0:
                return bot_repo.name();
            case 1:
                return bot_repo.platform();
            case 2:
                return bot_repo.developer();
            case 3:
                return bot_repo.description();
            case 4:
                return bot_repo.gitUrl();
        }
    }
...
QVariant BotRepoListModel::headerData(int section, Qt::Orientation orientation,
                                      int role) const {
...
bool BotRepoListModel::insertRows(int position, int rows,
...
bool BotRepoListModel::removeRows(int position, int rows,
                                  const QModelIndex &parent) {
...
bool BotRepoListModel::setData(const QModelIndex &index, const QVariant &value,
                               int role) {
    if (!index.isValid()) {
        return false;
    }
