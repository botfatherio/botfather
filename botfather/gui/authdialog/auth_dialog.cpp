...
AuthDialog::AuthDialog(LicenseApiClient *license_api, QWidget *parent)
    : QDialog(parent), ui(new Ui::AuthDialog), m_license_api(license_api) {
    ui->setupUi(this);
    int h = height();
    adjustSize();
    setFixedSize(width(), h);

    connect(m_license_api, &LicenseApiClient::networkError, this,
            &AuthDialog::onNetworkError);
    connect(m_license_api, &LicenseApiClient::errorsReceived, this,
            &AuthDialog::onErrorsReceived);
    connect(m_license_api, &LicenseApiClient::licenseReceived, this,
            &AuthDialog::onLicenseReceived);
    ...
    connect(ui->remember_me, &QCheckBox::toggled, this,
            &AuthDialog::rememberMe);
    ...
    bool remember_me =
        settings.value(auth::options::REMEMBER_ME, false).toBool();
...
void AuthDialog::login() {
    ...
    if (username.isEmpty()) {
        QMessageBox::warning(this, "Username missing",
                             "Username missing. Please fill out all fields.");
        return;
    }

    if (password.isEmpty()) {
        QMessageBox::warning(this, "Password missing",
                             "Password missing. Please fill out all fields.");
        return;
    }

    allowInput(
        false);  // Prevent spamming until we receive an reply from the server
    m_license_api->requestLicense(software, username, password);
}

void AuthDialog::logout() {
    // Forget about the user when he logs out. Otherwise he'll be automatically
    // logged in again when he start the application the next time.
    rememberMe(false);
}
...
void AuthDialog::rememberMe(bool checked) {
    ...
    if (!checked) {
        // Remove ones username and password from disk if they do no longer want
        // them to be saved.
...
void AuthDialog::onNetworkError(QNetworkReply::NetworkError network_error) {
    QString error_message =
        QString(
            "Network errors occured. Please check your Internet Connection. "
...
void AuthDialog::onErrorsReceived(QJsonArray error_codes) {
    static const QHash<QString, QString> error_messages = {
        {"username_missing",
         "Username missing... (this is a bug, please report it)."},
        {"password_missing",
         "Password missing... (this is a bug, please report it)."},
        {"software_missing",
         "Software missing... (this is a bug, please report it)."},
        {"software_unknown",
         "Software unknown... (this is a bug, please report it)."},
        {"user_auth_failed",
    ...
    for (QJsonValue error_code : error_codes) {
        ...
        message_box.setText(error_messages.value(
            error_code.toString(),
            error_code.toString()));  // Fallback to the error code
