...
BotConfigWidget::BotConfigWidget(Bot *bot, QWidget *parent)
    : QWidget(parent), ui(new Ui::BotConfigWidget) {
    ui->setupUi(this);
    QVBoxLayout *config_container_layout =
        new QVBoxLayout(ui->config_container);
...
void BotConfigWidget::renderConfig(BotConfig bot_config,
...
void BotConfigWidget::renderGroup(BotConfigGroup group,
                                  QVBoxLayout *vbox_layout) {
    ...
    for (const BotConfigOption &option : group.options()) {
        ...
        if (!option_p->isStored()) {
            // Store the fallback if there is no other value stored yet. When
            // there is no value stored the Engines ConfigAPI won't return a
            // value (not even the fallback)
...
void BotConfigWidget::renderOption(BotConfigOption *option_p,
    ...
    switch (option_p->type()) {
        ...
        default:
            qDebug() << "Unhandled switch case  in "
                        "'BotConfigWidget::renderBotConfigSetting'";
            break;
    }
}

void BotConfigWidget::renderBooleanOption(BotConfigOption *option_p,
                                          QFormLayout *form_layout) {
    ...
    connect(checkbox, &QCheckBox::toggled,
            [option_p](bool checked) { option_p->setValue(checked); });
}

void BotConfigWidget::renderNumberOption(BotConfigOption *option_p,
                                         QFormLayout *form_layout) {
    ...
    connect(spinbox, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            [option_p](double new_value) { option_p->setValue(new_value); });
}

void BotConfigWidget::renderIntegerOption(BotConfigOption *option_p,
                                          QFormLayout *form_layout) {
    ...
    connect(spinbox, QOverload<int>::of(&QSpinBox::valueChanged),
            [option_p](int new_value) { option_p->setValue(new_value); });
}

void BotConfigWidget::renderStringOption(BotConfigOption *option_p,
                                         QFormLayout *form_layout) {
    ...
    connect(lineedit, &QLineEdit::editingFinished,
            [lineedit, option_p]() { option_p->setValue(lineedit->text()); });
}

void BotConfigWidget::renderPasswordOption(BotConfigOption *option_p,
                                           QFormLayout *form_layout) {
    QLineEdit *lineedit = new QLineEdit();
    lineedit->setPlaceholderText(option_p->label());
...
void BotConfigWidget::renderChoiceOption(BotConfigOption *option_p,
                                         QFormLayout *form_layout) {
    ...
    connect(comboxbox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            [comboxbox, option_p]() {
