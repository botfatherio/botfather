...
QJsonValue BFConverter::CefV8ValueToQJsonValue(
    ...
    if (cef_v8_value->IsString()) {
        return QJsonValue(
            QString::fromStdString(cef_v8_value->GetStringValue().ToString()));
    }
    ...
    if (cef_v8_value->IsDate()) {
        ...
        QTime time(cef_time.hour, cef_time.minute, cef_time.second,
        cef_time.millisecond); return QJsonValue(QDateTime(date, time));
        */
    ...
    if (cef_v8_value->IsArray()) {
        ...
        for (int i = 0; i < cef_v8_value->GetArrayLength(); ++i) {
            array.append(
                BFConverter::CefV8ValueToQJsonValue(cef_v8_value->GetValue(i)));
        }
    ...
    if (cef_v8_value->IsObject()) {
        ...
        for (const CefString &key : keys) {
            ...
            object.insert(q_string_key, BFConverter::CefV8ValueToQJsonValue(
                                            cef_v8_value->GetValue(key)));
        }
        return QJsonValue(object);
    }
    ...
    if (cef_v8_value->IsFunction()) {
        return QJsonValue(
            QString::fromStdString(cef_v8_value->GetFunctionName().ToString()));
    }

    return QJsonValue(QJsonValue::Undefined);  // Fallback
}

QString BFConverter::CefV8ValueToCompactJsonQString(
    const CefRefPtr<CefV8Value> &cef_v8_value) {
...
QVariant BFConverter::CefV8ValueToQVariant(
    const CefRefPtr<CefV8Value> &cef_v8_value) {
    if (!cef_v8_value->IsValid()) {
    ...
    if (cef_v8_value->IsObject()) {
        ...
        for (const CefString &key : keys) {
            QString q_string_key = QString::fromStdString(key.ToString());
            object.insert(q_string_key, BFConverter::CefV8ValueToQVariant(
                                            cef_v8_value->GetValue(key)));
        }
        return object;
    }
...
CefRefPtr<CefBinaryValue> BFConverter::QVariantToCefBinaryValue(
...
QCborValue BFConverter::CefV8ValueToQCborValue(
    const CefRefPtr<CefV8Value> &cef_v8_value) {
    if (!cef_v8_value->IsValid()) {
    ...
    if (cef_v8_value->IsArray()) {
        ...
        for (int i = 0; i < cef_v8_value->GetArrayLength(); ++i) {
            array.append(
                BFConverter::CefV8ValueToQCborValue(cef_v8_value->GetValue(i)));
        }
        return array;
    }
    ...
    if (cef_v8_value->IsObject()) {
        ...
        for (const CefString &key : keys) {
            QString q_string_key = QString::fromStdString(key.ToString());
            object.insert(q_string_key, BFConverter::CefV8ValueToQCborValue(
                                            cef_v8_value->GetValue(key)));
        }
        return object;
    }
...
CefRefPtr<CefBinaryValue> BFConverter::QCborValueToCefBinaryValue(
