...
bool HelperApp::OnProcessMessageReceived(CefRefPtr<CefBrowser> browser,
    ...
    /** Executes javascript in the browsers V8 context, serializes the result
     *value and returns it to the requesting
     ** browser process.
     **
     ** Expects:
    ...
    if (message->GetName() == "eval_javascript") {
        ...
        CefRefPtr<CefV8Exception> cef_v8_exception =
            CefRefPtr<CefV8Exception>();
        CefRefPtr<CefV8Value> cef_v8_result_value = CefRefPtr<CefV8Value>();
        bool success = context->Eval(javascript_code, script_url, start_line,
                                     cef_v8_result_value, cef_v8_exception);

        // Prepare the result process message
        CefRefPtr<CefProcessMessage> result_msg =
            CefProcessMessage::Create("eval_javascript_result");
        ...
        if (success) {
            QCborValue cbor_result =
                BFConverter::CefV8ValueToQCborValue(cef_v8_result_value);
            CefRefPtr<CefBinaryValue> binary_result =
                BFConverter::QCborValueToCefBinaryValue(cbor_result);
            ...
            map["message"] = QString::fromStdString(
                cef_v8_exception->GetMessage().ToString());
            map["script_resource_name"] = QString::fromStdString(
                cef_v8_exception->GetScriptResourceName().ToString());
            map["source_line"] = QString::fromStdString(
                cef_v8_exception->GetSourceLine().ToString());
            map["start_column"] = cef_v8_exception->GetStartColumn();
            map["start_position"] = cef_v8_exception->GetStartPosition();

            CefRefPtr<CefBinaryValue> binary_exception =
                BFConverter::QVariantToCefBinaryValue(map);
            result_msg_args->SetBinary(3, binary_exception);
        }

        // We must stay in context while parsing/accessing any V8 values,
        // otherwise the renderer process will crash with "Check failed:
        // Currently not in a V8 context".
        context->Exit();
